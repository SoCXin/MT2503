...
//	if(systemset.Interval>=3600 || module_data.csq < 12) //当上传间隔小于 1小时时 使用 gsm  信号判断是否使用gps
	...
	*/{
	...
	Ql_strcat(alinkvaluebuf,tempbuf);
	Ql_sprintf(tempbuf,"\"gyr\":\"%d-%d-%d\",",mpu6050data.gx,mpu6050data.gy,mpu6050data.gz);
	Ql_strcat(alinkvaluebuf,tempbuf);
	Ql_sprintf(tempbuf,"\"acc\":\"%d-%d-%d\"",mpu6050data.ax,mpu6050data.ay,mpu6050data.az);
	Ql_strcat(alinkvaluebuf,tempbuf);
	...
	if(p!=NULL)
		{
			...
			ret=Ql_strcmp(key,"sportlev");
			if(ret==0)
				{
					SPORTLEVEL=strtol((const char*)value,NULL,10);
				}
